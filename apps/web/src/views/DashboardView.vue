  ...
  <section class="page-stack">
    ...
    <template v-else-if="dashboard && service">
      ...
      <div class="dashboard-grid">
        ...
        <article class="panel consumption-panel"><header><div><small>CONSUMPTION</small><h2>消费账户概览</h2></div><RouterLink to="/consumption">查看分析 →</RouterLink></header><div class="dashboard-consumption"><span>最近 14 天金额</span><strong>{{ compact(dashboard.kpis.currentConsumption) }}</strong><small>{{ dashboard.kpis.currentConsumption == null ? '同步消费快照后自动展示' : '独立消费账户，不关联售后客户档案' }}</small><RouterLink class="ghost-button" to="/consumption">进入消费分析</RouterLink></div></article>
      </div>
