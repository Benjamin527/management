...
const { listServiceRecords, getServiceRecord } = vi.hoisted(() => ({
  listServiceRecords: vi.fn(),
  getServiceRecord: vi.fn(),
}));
vi.mock("../../api/serviceRecords", () => ({
  listServiceRecords,
  getServiceRecord,
}));
...
async function mountView(path = "/service-records") {
  const router = createRouter({
    ...
    routes: [{ path: "/service-records", component: ServiceRecordsView }],
  });
  await router.push(path);
  const wrapper = mount(ServiceRecordsView, {
    global: {
      plugins: [router],
      stubs: { SyncStatusBar: true, teleport: true },
    },
  });
...
describe("ServiceRecordsView", () => {
  beforeEach(() => {
    ...
    getServiceRecord.mockResolvedValue({
      ...row,
      conclusion: "已提供脚本",
      rawFields: { 状态: "已提交飞书项目" },
      sourceUrl: "https://example.feishu.cn/wiki/example?record=rec1",
    });
  });

  it("hydrates drill-down filters from the URL", async () => {
    await mountView(
      "/service-records?status=ESCALATED&dateFrom=2026-08-01&dateTo=2026-08-31",
    );
    await flushPromises();
