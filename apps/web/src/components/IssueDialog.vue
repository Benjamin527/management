...
watch(
  ...
  async (open) => {
    if (!open) return;
    Object.assign(form, {
      serviceNo: String(Date.now()).slice(-6),
      customerId: "",
      title: "",
      description: "",
      channel: "FEISHU" as IssueChannel,
...
<template>
  <Teleport to="body"
    ><div v-if="open" class="dialog-backdrop" @click.self="emit('close')">
      <section
        class="dialog-card issue-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <small>NEW SERVICE ISSUE</small>
            <h2 id="issue-dialog-title">登记服务问题</h2>
          </div>
        ...
        <form data-form="issue" @submit.prevent="submit">
          ...
          <label class="full-field"
            ...
            ><textarea
              ...
              name="description"
              rows="5"
              placeholder="记录现象、影响范围和客户诉求"
            ></textarea>
          </label>
          <label
            >反馈渠道<select v-model="form.channel" name="channel">
              <option value="FEISHU">飞书</option>
              <option value="WECHAT">微信</option>
              <option value="DINGTALK">钉钉</option>
              <option value="PHONE">电话</option>
              <option value="EMAIL">邮件</option>
              <option value="FORM">服务表单</option>
              <option value="OTHER">其他</option>
            </select></label
          >
          <label
            >优先级<select v-model="form.priority" name="priority">
