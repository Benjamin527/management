...
export class IssuesController {
  ...
  @Post() create(
    ...
    return this.issues.create(input, user.sub);
  }
  ...
  @Post(':id/assign') assign(
    @Param('id') id: string,
    @Body() input: AssignIssueDto,
    @CurrentUser() user: SessionUser,
  ) {
    return this.issues.assign(id, input.assigneeId, user.sub);
  }
  @Post(':id/status') status(
    ...
    return this.issues.changeStatus(id, input.status, user.sub, input.comment);
  }
}
